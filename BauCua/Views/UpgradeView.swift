        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 22) {
                    ...
                    Text(L(purchases.trialActive ? "upgrade.subtitle" : "upgrade.subtitle.trialended"))
                        .font(.subheadline).foregroundStyle(.white.opacity(0.75))
                        .multilineTextAlignment(.center).padding(.horizontal, 30)

                    VStack(alignment: .leading, spacing: 12) {
                        featureRow("paintpalette.fill", L("upgrade.feature.themes"))
                        featureRow("chart.bar.fill", L("upgrade.feature.stats"))
